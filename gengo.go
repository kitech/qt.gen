...
func (this *GenerateGo) genFileHeaderWithcp(cp *CodePager, cursor, parent clang.Cursor) {
	file, line, col, _ := cursor.Location().FileLocation()
	if false {
		log.Printf("%s:%d:%d @%s\n", file.Name(), line, col, file.Time().String())
	}
...
func (this *GenerateGo) isinnomin(cursor clang.Cursor) bool {
	file, line, col, _ := cursor.Location().FileLocation()
	if false {
		log.Printf("%s:%d:%d @%s\n", file.Name(), line, col, file.Time().String())
	}
...
func (this *GenerateGo) genImportsWithcp(cp *CodePager, cursor, parent clang.Cursor) {
...
func ismthnomin(cursor clang.Cursor) bool {
	file, _, _, _ := cursor.Location().FileLocation()
...
func (this *GenerateGo) getpropercp(cursor clang.Cursor) *CodePager {
...
// TODO common can move to base
// a little about comment block, some #, ; ...
func (this *GenerateGo) genMethodHeader(cursor, parent clang.Cursor, midx int) {
	...
	cp.APf("body", "// retlen[%d] %s %s%s", cursor.ResultType().SizeOf(),
...
func (this *GenerateGo) genMethodSignature(cursor, parent clang.Cursor, midx int) {
	...
	default:
		...
		tycitm := getTyDescV2(cursor.ResultType(), cursor, LNGo)
		retPlace = tycitm.AsArgSign
...
func (this *GenerateGo) genCtor(cursor, parent clang.Cursor, midx int) {
	...
	convlines := this.genArgs(cursor, parent, midx, -1)
	this.outcodes(cursor, convlines)
	...
	cp.APf("body", "    gothis := New%sFromPointer(unsafe.Pointer(uintptr(rv)))", parent.Spelling())
...
func (this *GenerateGo) genCtorDv(cursor, parent clang.Cursor, midx int, dvidx int) {
	...
	convlines := this.genArgs(cursor, parent, midx, dvidx)
	this.outcodes(cursor, convlines)
...
func (this *GenerateGo) genDtor(cursor, parent clang.Cursor, midx int) {
	...
	cp.APf("body", "    rv, err := qtrt.InvokeQtFunc6(\"%s\", qtrt.FFI_TYPE_VOID, this.GetCthis())",
...
func (this *GenerateGo) genDtorNoCode(cursor, parent clang.Cursor, midx int) {
	...
	cp.APf("body", "    rv, err := qtrt.InvokeQtFunc6(\"_ZN%d%sD2Ev\", qtrt.FFI_TYPE_VOID, this.GetCthis())",
...
func (this *GenerateGo) genNonStaticMethod(cursor, parent clang.Cursor, midx int) {
	...
	if retype.Kind() == clang.Type_Record &&
		...
		cp.APf("body", "    rv, err := qtrt.InvokeQtFunc6(\"%s\", %s %s, this.GetCthis(), %s)",
...
func (this *GenerateGo) outcodes(cursor clang.Cursor, codes []string) {
	var cp = this.getpropercp(cursor)
...
func (this *GenerateGo) genArgs(cursor, parent clang.Cursor, midx int, dvidx int) (convcodes []string) {
	// return values
	...
	for idx := 0; idx < int(cursor.NumArguments()); idx++ {
		...
		for _, line := range codes {
			convcodes=append(convcodes, line)
		}
	}
	convcodes = append(convcodes,
		fmt.Sprintf("    // %s // prmlst4ffi", strings.Join(this.argDesc, ", ")))
...
func (this *GenerateGo) genArg(cursor, parent clang.Cursor, idx int, dvidx int, aitm *GenArgItem) (convcodes []string) {
	...
	switch aitm.tycv_item.ToFfiCvty {
		...
		case Fcty_Charptr:
			...
			cp.APf("body", "    var %s = %sNewQString9(%s)", aitm.tmpname, pkgPref, aitm.oriname)
			cp.APf("body", "    var %s = %s.ToUtf8().Data()", aitm.convname, aitm.tmpname)
			cp.APf("body", "    defer %s.Dtor()", aitm.tmpname)
			aitm.ffiprm = aitm.convname
		default: {

		}
	}
	convcodes = strings.Split(cp.ExportAll(), "\n")
	convcodes = convcodes[1:len(convcodes)-2]
...
func (this *GenerateGo) genRetFFI(cursor, parent clang.Cursor, midx int) {
	...
	{
		rety := cursor.ResultType().RemoveLocalConst()
		if rety.Kind() == clang.Type_Void {
			return
		}

		tycitm := getTyDescV2(rety, cursor, LNGo)
		cp.APf("body", "    // %s, %s, %s", rety.Spelling(), tycitm.AsArgSign, tycitm.AsFfiCall)

		switch tycitm.ToFfiCvty {
			default :
...
func (this *GenerateGo) genFunctions(cursor clang.Cursor, parent clang.Cursor) {
	...
	for _, qtmod := range qtmods {
		...
		for _, fc := range funcs {
			...
			if skip, reason := this.filter.skipFunc(fc); skip {
				log.Println("skip global function ", fc.Spelling(), reason)
				continue
			}
